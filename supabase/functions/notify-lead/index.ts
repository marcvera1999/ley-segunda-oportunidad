  ...
  try {
    ...
    if (!EMAIL_VIA_RESEND_ENABLED) {
      ...
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1a2942;">
          <h2 style="color: #1a2942; border-bottom: 2px solid #c9a84c; padding-bottom: 10px;">
            🔔 Nueva consulta – Deuda Eliminada
          </h2>
          <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
            <tr><td style="padding: 8px 0; font-weight: bold; width: 180px;">Nombre:</td><td>${escapeHtml(lead.nombre)}</td></tr>
            <tr><td style="padding: 8px 0; font-weight: bold;">Teléfono:</td><td>${escapeHtml(lead.contacto_valor)}</td></tr>
      ...
      const emailBody: Record<string, unknown> = {
        from: "Deuda Eliminada <onboarding@resend.dev>",
        to: ["javiermassana@gmail.com"],
        subject: "🔔 Nueva consulta – Deuda Eliminada",
        html,
      };
    ...
    if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_PHONE_NUMBER) {
      console.error("Missing Twilio credentials (SID / token / TWILIO_PHONE_NUMBER)");
    } else {
      const smsBody = `🔔 Nueva consulta Deuda Eliminada
Nombre: ${lead.nombre}
Teléfono: ${lead.contacto_valor}`;
